...
function updatePlanPrices() {
    ...
    if (document.getElementById('weeklyPrice')) {
...
async function updateCurrentPlanStatus() {
    ...
    if (subscription.active) {
        ...
        statusElement.innerHTML = `
            ...
            <strong>You are currently a Premium Member (${subscription.plan.toUpperCase()})</strong> - Active until ${expiryDate}
            <br><small>You can extend or upgrade your plan anytime</small>
        `;
...
function selectPlan(planType) {
    ...
    document.querySelectorAll('.plan-card').forEach(card => {
        ...
        const label = card.querySelector('.plan-selector label');
        if (label) {
    ...
    const radio = document.getElementById(`${planType}Plan`);
